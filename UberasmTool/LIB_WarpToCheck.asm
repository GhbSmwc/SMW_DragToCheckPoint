;Have gamemode14 or levelXXX call using [JSL LIB_WarpToCheck_WarpToLastCheckpointRoutineStart] under [main:].

incsrc "../WarpToCheckPointDefines/Defines.asm"

WarpToLastCheckpointRoutineStart:
	.WarpToLastCheckpoint
	LDA $71						;\If player is dying, don't interrupt it
	CMP #$09					;|
	BNE +						;/
	RTL
	+
	LDA !Freeram_WarpToLastCheckpoint_Status	;\If Mario in normal state, don't do anything.
	BNE +						;|
	RTL						;/
	+
	CMP #$FF					;\State to drag player to last checkpoint
	BEQ ..DragPlayer				;/
	
	STZ $15						;\Disable controls
	STZ $16						;|
	STZ $17						;|
	STZ $18						;/
	
	..PriorDrag
	LDA $9D						;\Don't decrement when frozen.
	BNE +						;/
	LDA !Freeram_WarpToLastCheckpoint_Status	;\Decrement
	DEC						;|
	STA !Freeram_WarpToLastCheckpoint_Status	;/
	+
	LDA !Freeram_WarpToLastCheckpoint_Status	;\If not decrement to zero, continue to stun player.
	BNE +						;/
	LDA #$FF					;\Set state to drag player to checkpoint
	STA !Freeram_WarpToLastCheckpoint_Status	;/
	BRA ..DragPlayer				;>And do it on that frame.
	
	+
	STZ $7B						;\Freeze player in midair.
	STZ $7D						;/
	BRA ..Done
	
	..DragPlayer
	...CheckIfCheckpointReached
	JSL MarioNotCloseToCheckpoint
	BCS ...MovePlayer
	...
	STA $1497|!addr
	REP #$20
	LDA !Freeram_WarpToLastCheckpoint_LastXPos
	STA $94
	LDA !Freeram_WarpToLastCheckpoint_LastYPos
	STA $96
	SEP #$20
	BRA ..Done
	...
	STA $71						;|
	STA $9D						;/
	STA $185C|!addr					;>Go through walls and ignore layer interaction
	STA $1404|!addr					;>Make screen scroll up.
	REP #$20					;\Drag player to checkpoint
	LDA $94						;|
	SEC						;|
	SBC !Freeram_WarpToLastCheckpoint_LastXPos	;|
	STA $00						;|
	LDA $96						;|
	SEC						;|
	SBC !Freeram_WarpToLastCheckpoint_LastYPos	;|
	STA $02						;|
	SEP #$20					;|
	LDA #!WarpToLastCheckpoint_DragSpeed		;|
	JSL Aiming					;|
	LDA $00						;|
	STA $7B						;|
	LDA $02						;|
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;Check if mario and the checkpoint are close to one another within
;;8 pixels on both XY axis.
;;
;;Carry is set if too far away, otherwise clear when close.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
MarioNotCloseToCheckpoint:
	REP #$20						;\Absolute distance horizontal
	LDA $94							;|
	SEC							;|
	SBC !Freeram_WarpToLastCheckpoint_LastXPos		;|
	BPL +							;|
	...
	CMP #$0008
	BCS .Done
	LDA $96							;\Absolute distance vertical
	SEC							;|
	SBC !Freeram_WarpToLastCheckpoint_LastYPos		;|
	BPL +							;|
	EOR #$FFFF						;|
